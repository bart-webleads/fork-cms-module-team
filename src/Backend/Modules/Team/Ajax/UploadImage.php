    ...
    private function upload($file)
    {
        ...
        if($file_data)
        {
            ...
            if (filesize($temp_file) > 0)
            {
                ...
                $insert['team_member_id'] = $this->id;
                $insert['data'] = serialize(array());

                $insert['sequence'] = BackendTeamImagesModel::getMaximumSequence($this->id) + 1;
                $insert['hidden'] = 'N';

                //BackendTeamHelper::generateThumbnail($filename, $files_path, $preview_files_path);
                ...
                $insert['id'] = BackendTeamImagesModel::insert($insert);
                ...
                foreach($this->languages as $language)
                {
                 ...
                 // insert it
               BackendTeamImagesModel::insertContent($content);
