...
class Category
{
         ...
         const QRY_DATAGRID_BROWSE =
             'SELECT i.id, c.name,  i.sequence
              ...
              INNER JOIN team_category_content as c  on i.id = c.category_id
        ...
        public static function getURL($URL, $language, $id = null)
        {
            ...
            if ($id === null) {
                ...
                if ((bool) $db->getVar(
                    'SELECT 1
                     FROM team AS i
                     INNER JOIN team_category_content AS m ON i.id = m.category_id
        ...
        */
        ...
        {
           BackendModel::get('database')->delete('team_categories', 'id = ?', (int) $id);
           BackendModel::get('database')->delete('team_category_content', 'category_id = ?', (int) $id);
           BackendModel::get('database')->delete('team_linked_catgories', 'category_id = ?', (int) $id);
        }
        ...
        */
        public static function get($id)
        {
           ...
           $return['content'] = (array) $db->getRecords(
               'SELECT i.* FROM team_category_content AS i
               WHERE i.category_id = ?',
        ...
        public static function getForDropdown()
         {
             $allCategories = (array) BackendModel::get('database')->getRecords('SELECT i.id, c.name, i.parent_id
              FROM team_categories AS i
              INNER JOIN team_category_content AS c on c.category_id = i.id
               WHERE c.language = ?
        ...
        */
        public static function insert(array $item)
        {
           ...
           return (int) BackendModel::get('database')->insert('team_categories', $item);
        }
        ...
        public static function insertContent(array $content)
        {
            foreach($content as &$item){
                ...
                if( BackendModel::get('fork.settings')->get('Team', 'make_widget_per_category', true) == true)
                {
                    $data = [
                        'id' => $item['category_id'],
                ...
                BackendModel::get('database')->insert('team_category_content', $item);
            }
        }
        ...
        */
        public static function update(array $item)
        {
           ...
           BackendModel::get('database')->update(
               'team_categories', $item, 'id = ?', (int) $item['id']
           );
        }
        ...
        public static function updateContent(array $content, $id)
        {
           ...
           {
               if( BackendModel::get('fork.settings')->get('Team', 'make_widget_per_category', true) == true && $row['extra_id'])
               {
                   $data = [
                       'id' => $row['category_id'],
               ...
               $db->update('team_category_content', $row, 'category_id = ? AND language = ?', array($id, $language));
           }
        }
