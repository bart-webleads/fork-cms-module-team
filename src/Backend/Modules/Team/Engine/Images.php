...
class Images
{
      ...
      const QRY_DATAGRID_BROWSE_IMAGES_FOR_PROJECT  =
        ...
        public static function getURL($URL, $language, $id = null)
        {
            ...
            if ($id === null) {
                ...
                if ((bool) $db->getVar(
                    'SELECT 1
                     FROM team AS i
                     INNER JOIN team_member_images_content AS m ON i.id = m.image_id
        ...
        */
        ...
        {
           BackendModel::get('database')->delete('team_member_images', 'id = ?', (int) $id);
           BackendModel::get('database')->delete('team_member_images_content', 'image_id = ?', (int) $id);
        }
        ...
        */
        public static function get($id)
        {
           ...
           $return['content'] = (array) $db->getRecords(
               'SELECT i.* FROM team_member_images_content AS i
               WHERE i.image_id = ?',
        ...
        public static function getAll($id)
        {
           ...
           $return =  (array) $db->getRecords(
               'SELECT i.*
                FROM team_member_images AS i
                WHERE i.team_member_id = ? ORDER BY i.sequence',
               array((int) $id)
           );
        ...
        */
        public static function insert(array $item)
        {
           ...
           return (int) BackendModel::get('database')->insert('team_member_images', $item);
        }

        public static function insertContent(array $content)
        {
           BackendModel::get('database')->insert('team_member_images_content', $content);
        }
        ...
        */
        public static function update(array $item)
        {
           ...
           BackendModel::get('database')->update(
               'team_member_images', $item, 'id = ?', (int) $item['id']
           );
        }
        ...
        public static function updateContent(array $content, $id)
        {
           ...
           {
               $db->update('team_member_images_content', $row, 'image_id = ? AND language = ?', array($id, $language));
           }
        }
