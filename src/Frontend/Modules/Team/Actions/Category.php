...
class Category extends FrontendBaseBlock
{
    ...
    private function loadForm()
    {
        ...
        $categories = FrontendTeamCategoriesModel::getForMultiCheckboxForParent($this->record['id']);
    ...
    private function getData()
    {
        ...
        if (empty($parameter)) {
         ...
         $this->record = FrontendTeamCategoriesModel::get($parameter);
        ...
        if($this->filter['form'] == NULL) {
            $children = FrontendTeamCategoriesModel::getAllChildrenByPath( $this->record['path']);
        ...
        // set URL and limit
        $this->pagination['url'] = Navigation::getURLForBlock('Team') . '?' . http_build_query($this->filter);
        $this->pagination['limit'] = $this->get('fork.settings')->get('Team', 'overview_num_items', 10);
        $this->pagination['limit'] = 100;

        // populate count fields in pagination
        $this->pagination['num_items'] = FrontendTeamModel::getAllCount($this->filter);
    ...
    private function parse()
    {
        ...
        $allCategories = FrontendTeamCategoriesModel::getAll(array('parent_id' => 0));
        unset($allCategories[$this->record['id']]);
        $this->tpl->assign('widgetTeamCategories',$allCategories  );
